...
| **AI assistant** | Choose **Claude** (Anthropic), **ChatGPT** (OpenAI), **Gemini** (Google) or **Grok** (xAI) in Settings, with your own API key for that service. Every AI feature (tagging, smart search, flashcards, cases, hints, answer check, tutor) uses the one you pick. **Load models** lists the models your key can use, and **Test** checks the connection. Keys stay on the device. |
| **AI tagging** | The AI reads each question with its answer and explanation. It assigns a topic and subtopic from a built-in neurosurgery taxonomy (16 topics, ~80 subtopics), plus concept tags, search keywords and synonyms, a difficulty, a high-yield flag and a one-line teaching point. An offline keyword tagger runs on every import as a first pass. You can correct tags by hand. |
...
| **Flashcards** | Real flip cards: a study session opens as a full-screen card that turns over (tap, click or Space) to show the answer. Long answers, tables and images scroll inside the card. After flipping, swipe right for Good or left for Again, or use the buttons and keys `1`–`4`. Browse → **Flip** opens any card the same way, with ← → to move through the list. Cards come from imported flashcards, from questions (one tap), from Claude (short one-fact cards) or from you. Study with spaced repetition or cram mode. The scheduler is **FSRS-5** (fewer reviews than SM-2 for the same retention); SM-2 is available in Settings. |
...
| **Image atlas** | Standalone reference figures, tables and scans can be imported without creating quiz questions. Browse by topic, search titles and tags, or filter question, answer and case images. Source question and case images retain links to their study material. |
...
To use the AI features, pick a provider (Claude, ChatGPT, Gemini or Grok) in **Settings → AI assistant** and add your API key for it. The key stays on that device and is never synced. The default is Claude (`claude-opus-5`). For bulk tagging of large books you can switch to a cheaper model in Settings. Tagging runs in batches of 12 with the taxonomy prompt cached, and you can stop and resume it.
